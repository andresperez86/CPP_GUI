...
grid.assignDataGrid(path, 'BCD_Route'+str(step))
...
polygon = Polygon(np.array(coverage_path_wavefront))
polygon_points = np.array(polygon.exterior)
...
grid.assignDataGrid(path, 'WAVE_Route'+str(step))
grid.calculateStatistics()
...
polygon = Polygon(np.array(coverage_path_stc))
polygon_points = np.array(polygon.exterior)
...
grid.assignDataGrid(path, 'STC_Route'+str(step))
grid.calculateStatistics()
